...
public class AlertLogDetailActivity extends BaseMvpActivity<WorkOrderDetailContract.Presenter> implements WorkOrderDetailContract.View {

    @Override
    protected Pair<Integer, WorkOrderDetailContract.Presenter> layoutAndPresenter() {
        return Pair.create(R.layout.activity_alert_log_detail, new WorkOrderDetailPresenter(this));
    }

    public static void startActivity(Context context, AssetAlertBean it,  ActivityResultLauncher<Intent> launcher) {
        Intent intent = new Intent(context, AlertLogDetailActivity.class);
        Bundle bundle = new Bundle();
        bundle.putSerializable("AssetAlertBean", it);
    ...
    private AssetAlertBean mAssetAlertBean = null;
...
//        });
        ...
        if (bundle.containsKey("AssetAlertBean")) {
            mAssetAlertBean = (AssetAlertBean) bundle.getSerializable("AssetAlertBean");
        }

        LogUtils.json(mAssetAlertBean);
...
//        mWorkOrderBean = it;
        ...
        if (urls.size() > 0) {
            for (int j = 0; j < urls.size(); j++) {
                RadiusImageView riv = new RadiusImageView(AlertLogDetailActivity.this);
                ...
                riv.setOnClickListener(v -> new PicturePreviewDialog(AlertLogDetailActivity.this, url).show());
    ...
    public void workOrderNodeSuc(List<NodeVo> it) {
        ...
        for (int i = 0; i < it.size(); i++) {
            ...
            itv_edit.setOnClickListener(v -> {
                WorkOrderNodeEditActivity.startActivity(AlertLogDetailActivity.this, vo, mLauncher);
            });
            LogUtils.json(vo);
